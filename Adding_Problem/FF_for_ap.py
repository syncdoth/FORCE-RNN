    ...
    def train(self, inps_and_targs, monitor_training=0, **kwargs):
        ...
        *****NOTE***** The function inps_and_targs must have a third output of "hints" for training.
        ...
        try:
            ...
            num_targs = targs.shape[2]
            num_hints = hints.shape[2]
        ...
        for epoch in range(hp['num_epochs']):
            print(f'epoch {epoch+1} of {hp["num_epochs"]}, ',
                  f'{inputs.shape[0]} examples: ')
            inp, targ, hint = inps_and_targs(dt=hp['dt'],
                                             **kwargs)[0:3]
            for trial in range(inputs.shape[0]):
                if np.mod(trial, int(inputs.shape[0] / 100)) == 0:
                    print(f'progress.. {trial}/{inputs.shape[0]}')
            ...
            if epoch == 0:
                ...
                tvec = np.arange(0, inp.shape[0]) * hp['dt']
