        ...
        *****NOTE***** The function inps_and_targs must have a third output of "hints" for training.
        ...
        for epoch in range(hp['num_epochs']):
            ...
            for trial in range(len(inputs)):
                if np.mod(trial, int(len(inputs) / 10)) == 0:
                    # print(f'progress.. {trial}/{len(inputs)}')
                    print('.', end='')
                ...
                for t in range(batch['inp'].shape[0]):
                    ...
                    if t % hp['ff_steps_per_update'] == 0:
                        ...
                        rd = self.activations['tanh'](DRNN.x).T # [N, 1]
                        J = self.J.T           # [N, N]
                        w = self.w.T           # [1, N]
                        ...
                        if monitor_training == 1:
                            ...
                            w_error.append(np.sum(np.abs(w_err)) / w_err.shape[0])
                        epoch_error.append(np.sum(np.abs(w_err)) / w_err.shape[0])
                z_whole.append(z)
                dx_whole.append(dx)
                x_whole.append(x)
            ########## Epoch callback
            print('mean epoch error: ', sum(epoch_error) / len(inputs))
            print('')  # New line after each epoch

            # Convert lists to arrays
            # dx_whole = np.array(dx_whole)
            # x_whole = np.array(x_whole)
            # z_whole = np.array(z_whole)
            # print(z_whole.shape)
            ...
            #     tvec = np.arange(0, len(inputs)) * hp['dt']
            ...
            #                                 linestyle='--',
            #                                 color='r')
            #     lines_out = plt.Line2D(tvec, z_whole[:, -1], color='b')
            ...
            #                                                linestyle='--',
            #                                                color='r')
            #         lines_unit['%g' % i] = plt.Line2D(tvec, x_whole[:, -1, i], color='b')
            #         ax_unit.add_line(lines_targ_unit['%g' % i])
            #         ax_unit.add_line(lines_unit['%g' % i])

            #     # Set up the axes
            #     ax_out.set_xlim([0, hp['dt'] * inp.shape[0]])
            #     ax_unit.set_xlim([0, hp['dt'] * inp.shape[0]])
            #     ax_out.set_ylim([-1.2, 1.2])
            #     ax_unit.set_ylim([-2, 10])
            #     ax_out.set_title('Output')
            #     ax_unit.set_title('Recurrent units, epoch %g' % (epoch + 1))
            ...
            #     lines_targ_out.set_ydata(targ[:, -1, :])
            #     lines_out.set_xdata(tvec)
            #     lines_out.set_ydata(z[:, -1])
            #     for i in range(2):
            #         lines_targ_unit['%g' % i].set_xdata(tvec)
            #         lines_targ_unit['%g' % i].set_ydata(dx_whole[:, -1, i])
            #         lines_unit['%g' % i].set_xdata(tvec)
            #         lines_unit['%g' % i].set_ydata(x_whole[:, -1, i])
